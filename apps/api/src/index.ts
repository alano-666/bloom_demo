...
app.post("/api/onboarding", async (req, res) => {
  const input = onboardingSchema.parse(req.body satisfies OnboardingInput);
...
app.patch("/api/profile", async (req, res) => {
  const input = z
    .object({
      ...
      mainProblem: z.string().min(2),
    })
    .parse(req.body satisfies UpdateProfileInput);
...
app.get("/api/session/threads", (_req, res) => {
  res.json(demoStore.listThreads());
});

app.post("/api/session/threads", (req, res) => {
  const body = z.object({ title: z.string().min(1) }).parse(req.body satisfies ThreadCreateInput);
  res.json(demoStore.createThread(body.title));
});

app.get("/api/session/:threadId", (req, res) => {
  res.json(demoStore.getSession(req.params.threadId));
});
...
app.post("/api/session/message", async (req, res) => {
  const body = z
    ...
    .parse(req.body satisfies SessionMessageInput);
  res.json(await demoStore.postMessage(body));
});

app.get("/api/trajectory", (_req, res) => {
  res.json(demoStore.buildTrajectory());
});
...
app.post("/api/dashboard/core-task/generate", async (_req, res) => {
  res.json(await demoStore.generateCoreTask());
});

app.post("/api/dashboard/core-task/decompose", async (_req, res) => {
  res.json(await demoStore.decomposeTask());
});
...
app.delete("/api/dashboard/schedule/:taskId", (req, res) => {
  res.json(demoStore.deleteScheduleItem(req.params.taskId));
});

app.get("/api/goals", (_req, res) => {
  res.json(demoStore.getState().goals);
});
...
app.post("/api/goals", (req, res) => {
  const body = z
    .object({
      ...
      category: z.enum(["职业", "健康", "学习", "生活"]),
      targetDate: z.string().min(1),
      note: z.string().min(1),
    })
    .parse(req.body satisfies CreateGoalInput);
...
app.post("/api/goals/:goalId/progress", (req, res) => {
  const body = z
    .object({
      note: z.string().min(1),
      progressDelta: z.number().min(1).max(20),
    })
    .parse(req.body satisfies GoalProgressInput);
...
app.get("/api/settings", (_req, res) => {
  res.json(demoStore.getState().settings);
});
...
app.patch("/api/settings", (req, res) => {
  const body = z
    .object({
      ...
      personalizedRhythm: z.boolean().optional(),
      darkMode: z.boolean().optional(),
      fontScale: z.enum(["小", "中", "大"]).optional(),
      replyStyle: z.enum(["治愈陪伴", "结构清晰", "精准鼓励"]).optional(),
    })
    .parse(req.body);
