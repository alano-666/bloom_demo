...
const DEFAULT_BASE_URL = process.env.ALIBABA_BAILIAN_BASE_URL ?? "https://ws-9wb6f65uvmdt19ua.cn-beijing.maas.aliyuncs.com/compatible-mode/v1";
const DEFAULT_MODEL = process.env.ALIBABA_BAILIAN_MODEL ?? "qwen-plus";
...
async function requestJson<T>(prompt: string): Promise<T> {
  ...
  if (!apiKey) {
    throw new Error("Missing ALIBABA_BAILIAN_API_KEY");
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(`Bailian API error ${response.status}: ${text}`);
  }
